 ...
 */
export function checkAttestationVersion(schema, version) {
    if (schema && schema !== SAVE_SCHEMA) {
        throw new Error(`Incompatible attestation schema: "${schema}" (this library supports "${SAVE_SCHEMA}")`);
    }
    if (version) {
        const attMajor = version.split('.')[0];
        const libMajor = SAVE_VERSION.split('.')[0];
        if (attMajor !== libMajor) {
            throw new Error(`Incompatible attestation version: "${version}" (this library is v${SAVE_VERSION}, major version ${libMajor})`);
        }
    }
}
