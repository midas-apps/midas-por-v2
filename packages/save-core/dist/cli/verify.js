...
async function main() {
    try {
        ...
        if (options.publish) {
            ...
            console.log('='.repeat(50));
            if (!options.registry)
                throw new Error('--registry is required with --publish');
            if (!options.rpcUrl)
                throw new Error('--rpc-url is required with --publish');
            if (!options.privateKey)
                throw new Error('--private-key is required with --publish');
            if (!proofId)
                throw new Error('Attestation does not contain a proofId. Cannot publish to registry.');
            // Push to IPFS
            ...
            console.log('');
            const verificationCid = await pushToIpfs(verification, ipfsUrl, options.ipfsUsername, options.ipfsPassword);
            ...
            const latestAttestation = await fetchLatestAttestation(options.registry, proofId, options.rpcUrl);
            ...
            verification.metadata.attestationHash = latestAttestation.attestationHash;
            ...
            await setVerification(options.registry, proofId, latestAttestation.attestationHash, verificationHash, options.rpcUrl, options.privateKey);
