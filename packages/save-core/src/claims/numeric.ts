 ...
 * ```
 */
export function createNumericClaim(
  options: CreateNumericClaimOptions
): NumericClaimContent {
  return {
    id: options.id,
    claimType: 'inline',
 ...
 * - Aggregated: computed from other claims using aggregation functions
 */
export class NumericClaim implements IClaim {
  private readonly _id: ClaimId;
  private readonly _description?: string;
  private readonly _data: NumericData | ClaimPointer | undefined;
  private readonly _proof?: ProofData;
  private readonly _aggregation?: NumericAggregation;
  private readonly _unit?: string; // Default unit for source-backed resolution
  private _resolvedValue?: number; // Store resolved value separately for source-backed claims

  private constructor(
    id: ClaimId,
    data: NumericData | ClaimPointer | undefined,
    description?: string,
    proof?: ProofData,
    aggregation?: NumericAggregation,
    unit?: string
