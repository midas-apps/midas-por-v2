 ...
 */
export function extractFromStructuredText(
 ...
 */
export function resolveObjectPointer(data: Record<string, unknown> | StructuredTextData, pointer: string): unknown {
 ...
 * ]
 */
function parseTableFormat(text: string): any[] {
  const records: any[] = [];
 ...
 */
function parseTransformation(part: string): { function: string; field: string } | null {
  if (!part.startsWith('@')) {
    return null;
  }
 ...
 */
function resolveJsonPath(data: Record<string, unknown> | StructuredTextData, path: string): unknown {
  if (!path.startsWith('/')) {
 ...
 */
function isStructuredTextData(value: unknown): value is StructuredTextData {
