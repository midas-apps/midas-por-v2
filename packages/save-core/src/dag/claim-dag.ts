...
import type { ClaimId, ClaimData, NumericData, ObjectClaimData, StructuredTextData, IClaim } from '../types/index.js';
...
import { isClaimPointer, isObjectClaim } from '../types/claim.js';
import { extractFromStructuredText, resolveObjectPointer } from '../claims/structured-text.js';
...
export class ClaimDAG {
  private readonly claims: Map<ClaimId, Claim> = new Map();
  private readonly dependents: Map<ClaimId, Set<ClaimId>> = new Map();
  private readonly values: Map<ClaimId, number> = new Map(); // Store resolved numeric values for aggregations
    ...
    if (claim.dependencies.length > 0) {
      for (const depId of claim.dependencies) {
        if (!this.claims.has(depId)) {
          throw new Error(
            `Claim ${claim.id} depends on ${depId} which has not been added yet. ` +
        ...
        if (claim.isComposite && !dep.isResolved && dep.isComposite) {
          throw new Error(
            `Claim ${claim.id} depends on ${depId} which is not yet resolved. ` +
  ...
  hasDependenciesSatisfied(id: ClaimId): boolean {
    ...
    if (claim.isPrimitive) return true;
  ...
  private resolveClaim(claim: Claim): void {
    ...
    const objectClaimMap = new Map<string, ObjectClaim>();
    ...
    // Resolve aggregation claims
      ...
      const computedValue = executeNumericAggregation(
        claim.aggregation,
        this.values
      );
   ...
   */
  private resolvePointer(pointer: string, objectClaimMap: Map<string, ObjectClaim>, defaultUnit?: string): NumericData {
   ...
   * @returns NumericData
   */
  private convertToNumericData(value: unknown, pointer: string, defaultUnit?: string): NumericData {
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      if (typeof obj.value === 'number' && typeof obj.unit === 'string') {
        return {
          value: obj.value,
          unit: obj.unit,
          asset: typeof obj.asset === 'object' && obj.asset !== null ? obj.asset as NumericData['asset'] : undefined,
        };
      }
    }
    ...
    if (!defaultUnit) {
      throw new Error(
        `Pointer ${pointer} resolved to a plain value without a unit. ` +
   ...
   */
  private resolveStringPointer(pointer: string, objectClaimMap: Map<string, ObjectClaim>): string {
    const { sourceClaimId: claimId, path: pointerPath } = parseClaimPointer(pointer);
   ...
   */
  private hasCycle(startId: ClaimId): boolean {
    const visited = new Set<ClaimId>();
    const recursionStack = new Set<ClaimId>();

    const dfs = (id: ClaimId): boolean => {
      visited.add(id);
