 ...
 */
export function sum(...operands: AggregationOperand[]): SumAggregation {
 ...
 */
export function subtract(...operands: AggregationOperand[]): SubtractAggregation {
 ...
 */
export function executeNumericAggregation(
  aggregation: NumericAggregation,
  values: Map<ClaimId, number>
): number {
...
function executeWithDepth(
  ...
  if (depth >= MAX_AGGREGATION_DEPTH) {
    throw new Error(
      `Maximum aggregation nesting depth (${MAX_AGGREGATION_DEPTH}) exceeded`
    );
  }

  // Resolve each operand (either claim value or nested aggregation)
  const resolvedValues = aggregation.operands.map(operand =>
    resolveOperandWithDepth(operand, values, depth + 1)
  );
  ...
  switch (aggregation.function) {
    ...
    default:
      throw new Error(`Unknown aggregation function: ${(aggregation as any).function}`);
  }
}
 ...
 */
function resolveOperandWithDepth(
